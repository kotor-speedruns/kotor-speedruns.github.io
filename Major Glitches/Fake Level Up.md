...
All FLUs in speedruns begin with an [Anywhere Menu Glitch](d4jq8) (AMG), usually performed with a Quick Save.  After the [AMG](d4jq8), open the Character Screen.  You should see the following: 
...
[center]**Feats, Force Powers, attribute points and skill points assigned during Intermediate Levels *will not be permanent*.**[/center] 
...
[section=Skeleton Level (or Skele FLU)]
...
2. Perform an [AMG](d4jq8) with a Quick Save.  This should be skipped if your next level up is the Final Level.
...
[section=Slow Level (or Slow FLU)]
...
This will put you back on the Character screen with your SC active and no menu glitch.  To continue a FLU, swap back to the TC and exit the menu to [AMG](d4jq8) again.  If this is your Final Level, you can swap to the TC and perform the Final Level.
...
* Does not accumulate skill points
* Requires redoing [AMG](d4jq8) to continue the FLU

All speedrun FLUs do at least one or two Slow FLU levels to gain required prerequisites for the Final Level.

[section=Fast Level (or Fast FLU)]

This method allows for Intermediate Levels to be done in the same way as leveling up normally, without the use of an [AMG](d4jq8).  However, this can only be used under the following conditions:

* The first level of the FLU was an attribute level (4,8,12,16, or 20)
* It is not the first level of the FLU; this must be done as a Slow FLU level with an [AMG](d4jq8)

https://youtu.be/9NZO51L_C_4

[small]*This video uses a [Container Menu Glitch](d4jq8#ch8Classic_Menu_Glitch) to start the FLU; an [AMG](d4jq8) should be used instead, and will not affect the rest of the FLU.*[/small]
...
**Obtains:** Sneak Attack IX and Master Two-Weapon Fighting
...
[section=Other FLU Applications]
...
[section=Notes on Feats and Powers]
